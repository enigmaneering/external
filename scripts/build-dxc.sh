...
if [ -n "$CROSS_COMPILE_TARGET" ] && [ "$CROSS_COMPILE_TARGET" = "aarch64" ]; then
    ...
    cmake .. \
        ...
        "-DCMAKE_CXX_FLAGS=-O2 -DNDEBUG -std=gnu++17 -include windows.h -include strsafe.h -include wrl/client.h -DCComPtr=ComPtr -DATL_NO_VTABLE= -D_ATL_DECLSPEC_ALLOCATOR= -Wno-unused-command-line-argument -Wno-invalid-specialization -Qunused-arguments" \
